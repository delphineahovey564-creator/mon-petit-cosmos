...
function ModuleNumbers() {
  ...
  const done = new Set(child.completedNumbers);
  const lastIdx = NUMBERS.reduce((acc, n, i) => (done.has(n) ? i : acc), -1);
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F0] pb-28">
      <header className="h-14 px-4 flex items-center justify-between bg-white border-b border-[#F3F4F6]">
        ...
        <h1 className="font-bold text-[18px] text-[#1A1A2E]">Les Chiffres</h1>
      ...
      <section className="mx-4 mt-4">
        <div className="relative overflow-hidden rounded-[24px] p-5" style={{ height: 160, background: "linear-gradient(135deg,#B5EAD7,#52C7A0)" }}>
          <div className="absolute" style={{ right: -10, top: -20, transform: "rotate(15deg)" }}>
            <span className="font-black text-white" style={{ fontSize: 130, opacity: 0.12, lineHeight: 1 }}>1</span>
          </div>
          <div className="absolute right-2 bottom-0"><Leo size={80} /></div>
          <div className="relative z-10">
            <span className="inline-block bg-white text-[#52C7A0] font-bold text-[11px] uppercase rounded-full px-2.5 py-1">Niveau 2</span>
            <h2 className="mt-1.5 text-white font-black text-[30px] leading-tight">Les Chiffres</h2>
            <p className="text-white/85 font-semibold text-[13px]">De 0 à 20, maîtrise les nombres !</p>
            <div className="mt-2.5 inline-block bg-white rounded-full px-3 py-1">
              <span className="text-[#52C7A0] font-bold text-[13px]">{child.completedNumbers.length} / 21 chiffres</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-4 mt-4 bg-white rounded-[20px] shadow-edu-card px-5 py-4 flex justify-around">
        <Stat icon={<Trophy size={20} color="#FF6B35" />} value={child.completedNumbers.length} label="MAÎTRISÉS" />
        ...
        {NUMBERS.map((n, i) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => {
                if (state === "locked") { alert("Continue pour débloquer ce chiffre !"); return; }
                nav({ to: "/module/numbers/number/$num", params: { num: String(n) } });
              }}
            ...
            >
              {state === "locked" ? <Lock size={16} color="#D1D5DB" /> : n}
              {state === "completed" && <Star size={10} fill="#FFE14D" color="#FFE14D" className="absolute top-1 right-1" />}
            </motion.button>
          );
        })}
      </div>
